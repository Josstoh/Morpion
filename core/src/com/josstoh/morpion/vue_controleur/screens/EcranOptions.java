...
public class EcranOptions implements Screen{
    ...
    public EcranOptions(final Jeu monJeu)
    {
        ...
        table.defaults().width(500).height(100);
        ...
        table.add(gs).row();
        succes = new TextButton("Succès",skin);
        succes.addListener(new ChangeListener() {
            public void changed(ChangeEvent event, Actor actor) {
                if(!Jeu.googleServices.isSignedIn()){
                    Jeu.googleServices.signIn();
                }
                    Jeu.googleServices.showAchievements();
            }
        });
        table.add(succes).row();
